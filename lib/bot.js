(function() {
  var client, descriptions, dispatch, env, error, fortune, github, handlers, hear, image, irc, irc_channels, irc_name, irc_server, listen, say, seen, success, sys, weather;
  irc = require('irc');
  sys = require('sys');
  env = process.env;
  ...
  hear(/roll me/i, function(message) {
    seen.setSeenUser(message.from, message.to);
    return say(message.to, "" + message.from + " rolls a six sided die and gets " + (Math.floor(Math.random() * 6) + 1));
  });
}).call(this);
